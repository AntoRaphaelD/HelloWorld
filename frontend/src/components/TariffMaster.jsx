...
import { 
    ...
    Save, CheckCircle2, BookOpen, Hash, Layers, Search, Filter
} from 'lucide-react';
...
const TariffMaster = () => {
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            if (isUpdate) await mastersAPI.tariffs.update(formData.id, formData);
    ...
    return (
        <div className="min-h-screen bg-slate-50 p-6 font-sans text-slate-900">
            ...
            {showSuccess && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/20 backdrop-blur-sm transition-all">
                    <div className="bg-white p-8 rounded-2xl shadow-2xl flex flex-col items-center gap-3 animate-in zoom-in duration-300">
                        ...
                        <h3 className="text-xl font-bold text-slate-800 uppercase italic">{successMessage}</h3>
                    </div>
                </div>
            )}

            {/* Header Section - Matched to ProductMaster */}
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
                        <BookOpen className="text-blue-600" /> Tariff Machine Master
                    </h1>
                    <p className="text-sm text-slate-500 tracking-tight">Manage HSN codes, commodities, and yarn classifications</p>
                </div>
                ...
                <div className="flex gap-2">
                    ...
                    {isSelectionMode ? (
                        <button 
                            ...
                            className={`px-5 py-2 border rounded-lg flex items-center gap-2 transition-all ${selectedIds.length > 0 ? 'bg-white border-red-200 text-red-600' : 'bg-white border-slate-100 text-slate-300'}`}
                        >
            ...
            {/* DYNAMIC FILTER BAR - Matched to ProductMaster Style */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    <div>
                        ...
                        >
                            ...
                            <option value="tariff_code">Ref Code</option>
                    ...
                    <div className="md:col-span-1">
                        ...
                        <div className="relative">
                            ...
                            <input 
                                ...
                                className="w-full border border-slate-200 pl-10 pr-4 py-2 rounded-lg text-sm outline-none focus:ring-1 focus:ring-blue-500" 
            ...
            {/* Table - Matched to ProductMaster Density & Header Color */}
                            ...
                            {loading ? (
                                ...
                                currentItems.map((item) => (
                                    <tr 
                                        ...
                                        className={`transition-colors cursor-pointer ${selectedIds.includes(item.id) ? 'bg-blue-50' : 'hover:bg-blue-50'}`}
                                    >
                                        {isSelectionMode && (
                                        ...
                                        <td className="p-4 text-sm font-mono text-slate-500">{item.tariff_code}</td>
                                        <td className="p-4 text-sm font-bold text-slate-700 uppercase">{item.tariff_name}</td>
                                        <td className="p-4 text-sm text-slate-600 font-mono italic">{item.tariff_no}</td>
                                        <td className="p-4 text-xs font-bold text-slate-500 uppercase">{item.commodity}</td>
                                        <td className="p-4 text-xs font-bold text-slate-400 uppercase">{item.product_type}</td>
                                        {!isSelectionMode && (
                                ...
                                <tr><td colSpan="7" className="p-20 text-center text-slate-400 font-bold uppercase tracking-widest opacity-20 text-xl">No Matching Records</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>

                {/* Footer Pagination - Matched to ProductMaster */}
                <div className="p-4 bg-slate-50 border-t flex items-center justify-between">
                    <span className="text-xs text-slate-500 font-semibold uppercase tracking-wider">
                        Total {filteredData.length} entries found
                    </span>
                    <div className="flex gap-2">
                        <button disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)} className="p-2 border rounded bg-white disabled:opacity-30 hover:bg-blue-50 transition-all shadow-sm">
                            <ChevronLeft size={16}/>
                        </button>
                        <div className="px-3 flex items-center text-xs font-bold text-slate-600">
                            {currentPage} / {totalPages}
                        </div>
                        <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(p => p + 1)} className="p-2 border rounded bg-white disabled:opacity-30 hover:bg-blue-50 transition-all shadow-sm">
            ...
            {/* Modal - Matched to ProductMaster Blue Header Style */}
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden animate-in fade-in zoom-in duration-200">
                        <div className="bg-blue-600 p-4 flex justify-between items-center text-white">
                            <h2 className="font-black uppercase tracking-tight">{formData.id ? 'Modify Tariff Record' : 'Add New Tariff'}</h2>
                        ...
                        <form onSubmit={handleSave} className="p-8">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                <div className="md:col-span-2 space-y-6">
                                    <div className="grid grid-cols-2 gap-6">
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-bold text-slate-400 uppercase">Ref Code</label>
                                            <input readOnly value={formData.tariff_code} className="w-full border border-slate-100 p-3 rounded-xl text-sm font-mono bg-slate-50 text-blue-600" />
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-bold text-slate-400 uppercase">HSN Number *</label>
                                            <input required value={formData.tariff_no} onChange={e => setFormData({...formData, tariff_no: e.target.value})} className="w-full border border-slate-200 p-3 rounded-xl text-sm font-bold focus:ring-1 focus:ring-blue-500 outline-none" placeholder="5205..." />
                                        </div>
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-bold text-slate-400 uppercase">Tariff Name/Description *</label>
                                        <input required value={formData.tariff_name} onChange={e => setFormData({...formData, tariff_name: e.target.value.toUpperCase()})} className="w-full border border-slate-200 p-3 rounded-xl text-sm font-black uppercase focus:ring-1 focus:ring-blue-500 outline-none" placeholder="ENTER DESCRIPTION" />
                                    </div>
                                    
                                    <div className="grid grid-cols-2 gap-6">
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-bold text-slate-400 uppercase">Product Type</label>
                                            <select value={formData.product_type} onChange={e => setFormData({...formData, product_type: e.target.value})} className="w-full border border-slate-200 p-3 rounded-xl text-sm font-bold bg-white">
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <select value={formData.commodity} onChange={e => setFormData({...formData, commodity: e.target.value})} className="w-full border border-slate-200 p-3 rounded-xl text-sm font-bold bg-white">
                                ...
                                {/* Sidebar Logic - Matched to ProductMaster Sidebar */}
                                <div className="bg-slate-900 rounded-2xl p-6 text-white flex flex-col justify-center gap-4">
                                    <div className="text-center space-y-1">
                                        <Hash size={32} className="text-blue-400 mx-auto mb-2" />
                                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Classification</p>
                                    </div>
                                    <div className="space-y-4">
                                        <div className="space-y-1">
                                            <label className="text-[9px] font-bold text-slate-500 uppercase">Fibre</label>
                                            <select value={formData.fibre} onChange={e => setFormData({...formData, fibre: e.target.value})} className="w-full bg-slate-800 border-none rounded-lg p-2.5 text-xs font-bold outline-none">
                                                <option value="COTTON">COTTON</option>
                                        ...
                                        <div className="space-y-1">
                                            <label className="text-[9px] font-bold text-slate-500 uppercase">Yarn Type</label>
                                            <select value={formData.yarn_type} onChange={e => setFormData({...formData, yarn_type: e.target.value})} className="w-full bg-slate-800 border-none rounded-lg p-2.5 text-xs font-bold outline-none">
                            ...
                            <div className="mt-10 pt-6 border-t flex justify-between items-center">
                                {formData.id && (
                                    <button type="button" onClick={() => {if(window.confirm('Delete?')) mastersAPI.tariffs.delete(formData.id).then(fetchRecords); setIsModalOpen(false);}} className="flex items-center gap-2 text-red-500 font-bold hover:bg-red-50 px-4 py-2 rounded-xl transition-all">
                                ...
                                <div className="flex gap-4 ml-auto">
                                    ...
                                    <button type="submit" disabled={submitLoading} className="bg-blue-600 hover:bg-blue-700 text-white px-10 py-3 rounded-xl font-black shadow-xl flex items-center gap-2 transition-all active:scale-95 disabled:opacity-50">
                                        <Save size={18}/> {submitLoading ? 'SAVING...' : 'SAVE TARIFF'}
                                    </button>
                                </div>
                            </div>
                        </form>
