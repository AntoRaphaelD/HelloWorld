...
import { graphqlAPI, mastersAPI } from '../service/api';
import { 
...
const ProductMaster = () => {
    ...
    const fetchRecords = async () => {
        ...
        try {
            const query = `
                query {
                    getProducts {
                        id product_code product_name short_description commodity 
                        commodity_code packing_type fibre wt_per_cone charity_rs 
                        no_of_cones_per_pack pack_nett_wt tariff_sub_head 
                        printing_tariff_sub_head_no printing_tariff_desc
                        product_type spinning_count_name converted_factor_40s
                        actual_count other_receipt roundoff mill_stock
                    }
                }
            `;
            const data = await graphqlAPI(query);
            setList(Array.isArray(data.getProducts) ? data.getProducts : []);
    ...
    const fetchLookups = async () => {
        try {
            const query = `
                query {
                    getTariffSubHeads { id tariff_code tariff_name tariff_no product_type fibre commodity }
                    getPackingTypes { id packing_type }
                }
            `;
            const data = await graphqlAPI(query);
            setTariffs(Array.isArray(data.getTariffSubHeads) ? data.getTariffSubHeads : []);
