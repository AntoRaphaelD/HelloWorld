...
const TransportMaster = () => {
    ...
    useEffect(() => { 
        ...
        resetFilters([
            ...
            { value: 'transport_code', label: 'Transport Code' },
    ...
    const filteredData = useMemo(() => {
        ...
        result.sort((a, b) => {
            let aVal, bVal;
            if (sortField === 'transport_code') {
                aVal = String(a.transport_code || '');
                bVal = String(b.transport_code || '');
                return sortOrder === 'asc'
                    ? aVal.localeCompare(bVal, undefined, { numeric: true, sensitivity: 'base' })
                    : bVal.localeCompare(aVal, undefined, { numeric: true, sensitivity: 'base' });
            } else if (sortField === 'transport_name') {
                aVal = String(a.transport_name || '');
                bVal = String(b.transport_name || '');
                ...
                <table className="w-full text-left">
                    <thead className="bg-slate-900 text-white text-[10px] font-bold uppercase tracking-wider">
                        <tr>
                            ...
                            <th className="p-3 cursor-pointer select-none" onClick={() => handleSort('transport_code')}>
                                Code {sortField === 'transport_code' ? (sortOrder === 'asc' ? '▲' : '▼') : ''}
                            </th>
                            <th className="p-3 cursor-pointer select-none" onClick={() => handleSort('transport_name')}>
                                Transport Name {sortField === 'transport_name' ? (sortOrder === 'asc' ? '▲' : '▼') : ''}
                            </th>
